...
from PySide6.QtWidgets import (
    ...
    QLabel, QFrame, QPushButton, QLineEdit, QDialog, QFormLayout,
    QMessageBox, QScrollArea, QComboBox, QDoubleSpinBox
)
...
def init_database():
    ...
    try:
        cur.execute("ALTER TABLE milk_sales ADD COLUMN customer_id INTEGER")
    except sqlite3.OperationalError:
        pass

    # -----------------------------------------------------
    # ADD 15 SAMPLE COWS IF DATABASE IS EMPTY
    # -----------------------------------------------------
...
def save_animal(animal_id, name, breed, milk_per_day):
...
def update_animal(animal_db_id, animal_id, name, breed, milk_per_day):
...
def save_milk_record(animal_id, morning, evening):
...
def load_milk_records():
    ...
    cur.execute("""
        SELECT animal_id, date, morning_milk,
               evening_milk, total_milk
        FROM milk_records
        ORDER BY id DESC
        LIMIT 20
    """)
...
def save_milk_sale(liters, price, customer_id=None):
...
def load_milk_sales():
    ...
    cur.execute("""
        SELECT sale_date, liters,
               price_per_liter, total_amount
        FROM milk_sales
        ORDER BY id DESC
        LIMIT 20
    """)
...
def delete_customer_from_database(customer_id):
...
class AddAnimalDialog(QDialog):
    ...
    def __init__(self, parent=None, animal=None):
        ...
        self.milk.setRange(0, 100)
        self.milk.setDecimals(1)
        self.milk.setSuffix(" L/day")

        layout.addRow("Animal ID:", self.animal_id)
        layout.addRow("Name:", self.name)
        layout.addRow("Breed:", self.breed)
        layout.addRow("Milk/Day:", self.milk)
        ...
        if animal:
            ...
            self.animal_id.setText(str(animal[1]))
            self.name.setText(str(animal[2]))

            index = self.breed.findText(str(animal[3]))

            if index >= 0:
                self.breed.setCurrentIndex(index)

            self.milk.setValue(float(animal[4] or 0))
...
class AddMilkDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Add Milk Record")
        self.setMinimumWidth(350)
        ...
        self.morning.setRange(0, 100)
        self.morning.setDecimals(1)
        self.morning.setSuffix(" L")

        self.evening = QDoubleSpinBox()
        self.evening.setRange(0, 100)
        self.evening.setDecimals(1)
        self.evening.setSuffix(" L")

        layout.addRow("Animal:", self.animal)
        layout.addRow("Morning Milk:", self.morning)
        layout.addRow("Evening Milk:", self.evening)
...
class AddSaleDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("Record Milk Sale")
        self.setMinimumWidth(350)
        ...
        layout.addRow("Milk:", self.liters)
        layout.addRow("Price/Liter:", self.price)
...
class DairyAppBuilder(QMainWindow):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Dairy Farm App Builder")
        ...
        version = QLabel("Dairy Farm App v1.0")
        ...
        header_title = QLabel("🐄 Dairy Farm")
        ...
        phone_layout.addWidget(self.scroll)
        ...
        phone_layout.addWidget(nav)

        main_layout.addWidget(left)
        main_layout.addWidget(phone)
    ...
    def show_home(self):
        ...
        profit = today_sales - today_expenses
        ...
        milk_btn = QPushButton("🥛 Add Milk Record")
        milk_btn.clicked.connect(self.add_milk)

        sale_btn = QPushButton("💰 Record Milk Sale")
        sale_btn.clicked.connect(self.add_sale)

        expense_btn = QPushButton("💸 Add Expense")
        expense_btn.clicked.connect(self.add_expense)

        self.content_layout.addWidget(milk_btn)
        self.content_layout.addWidget(sale_btn)
        self.content_layout.addWidget(expense_btn)

        self.content_layout.addSpacing(10)

        status = QLabel("Farm Status")
        ...
        self.content_layout.addWidget(total_text)
    ...
    def refresh_animals(self):
        ...
        if not hasattr(self, "animals_layout"):
            return
        ...
        if not animals:
            ...
            self.animals_layout.addWidget(empty)
        ...
        for animal in animals:
            ...
            self.animals_layout.addWidget(card)
    ...
    def add_animal(self):
        ...
        if dialog.exec():
            ...
            animal_id = dialog.animal_id.text().strip()
            name = dialog.name.text().strip()
            breed = dialog.breed.currentText()
            milk = dialog.milk.value()
    ...
    def add_customer(self):
        ...
        if dialog.exec():
            ...
            address = dialog.address.text().strip()
    ...
    def show_milk(self):
        ...
        self.content_layout.addWidget(total)

        records = load_milk_records()
    ...
    def add_milk(self):
        ...
        dialog = AddMilkDialog(self)

        if dialog.exec():

            animal_id = dialog.animal.currentData()

            morning = dialog.morning.value()
            evening = dialog.evening.value()
    ...
    def add_sale(self):
        ...
        if dialog.exec():
            ...
            liters = dialog.liters.value()
            price = dialog.price.value()
            customer_id = dialog.customer.currentData()
    ...
    # =====================================================
    # OVERRIDE HOME EXPENSE NAVIGATION
    # =====================================================
